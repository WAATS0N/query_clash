...
async function loadSchema() {
    ...
    try {
        ...
        document.getElementById('schemaList').innerHTML = `<div class="error-msg">Schema load failed: ${e.message}</div>`;
    }
}
...
async function loadInvestigations() {
    ...
    try {
        ...
        document.getElementById('investigationList').innerHTML = '<div class="error-msg">Failed to load directives</div>';
    }
}
...
function renderTabs() {
    ...
    terminals.forEach((t, index) => {
        ...
        div.innerText = `TERM_${t.id.toString().padStart(2, '0')}`;
...
function restoreActiveTerminal() {
    ...
    if (t.resultsHTML) {
        ...
        document.getElementById('resultsArea').innerHTML = '<div class="result-msg">READY. AWAITING INPUT.</div>';
    }
    document.getElementById('sqlEditor').focus();
}
...
function renderTable(data) {
    if (!data.results || data.results.length === 0) {
        document.getElementById('resultsArea').innerHTML = '<div class="result-msg">QUERY OK. NO DATA RETURNED.</div>';
        return;
    }
